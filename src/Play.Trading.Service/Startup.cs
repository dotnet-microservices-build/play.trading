    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddMongo()
                    ...
                    .AddJwtBearerAuthentication(); // we need this so as to be able to get the userId from the authorization context in the controller

            AddMassTransit(services);
            services.AddControllers(options => //allows the mappng in the AcceptedAtAction response to find the method
            {
                options.SuppressAsyncSuffixInActionNames = false;
            })
            .AddJsonOptions(options => options.JsonSerializerOptions.DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull);
            // .AddJsonOptions(options => options.JsonSerializerOptions.IgnoreNullValues);
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseAuthentication(); //postioned between UseRouting and UseEndpoints
        ...
        private void AddMassTransit(IServiceCollection services)
        {
            services.AddMassTransit(configure =>
            {
                ...
                {
                    sagaConfigurator.UseInMemoryOutbox(); //always do this to make sure state is saved first before message is sent
                })
                .MongoDbRepository(r =>
                {
